<script lang="ts">
    ...
    async function fetchCryptoData(sym: string) {
      if (!sym) return;
      
      try {
        const res = await fetch(`http://localhost:8000/crypto/${sym}`);
        if (res.ok) {
          ...
          searchResult = null;
          error = `Cryptocurrency ${sym.toUpperCase()} not found.`;
        } else {
          searchResult = null;
    ...
    $: {
      ...
      console.log('Symbol from URL changed:', symbol);
      if (symbol) {
