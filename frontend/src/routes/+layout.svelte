	...
	<div class="bg-muted/40 hidden border-r md:block">
		...
		<div class="flex-1">
			...
			{#each navigation as item}
				<a 
				href={item.href} 
				class="{$page.url.pathname === item.href ? 'bg-muted text-primary' : 'text-muted-foreground'} hover:text-primary flex items-center gap-3 rounded-lg px-3 py-2 transition-all"
				>
		...
		<div class="navbar-end">
			...
			{#if $currentUser}
				...
				<DropdownMenu.Content align="end">
					...
					<DropdownMenu.Item>
					<a href="/account" aria-current={$page.url.pathname === '/profile' ? 'page' : undefined}>
						<button type="button">Account</button>
					</a>
					</DropdownMenu.Item>
					<DropdownMenu.Item>
					<a href="/security" aria-current={$page.url.pathname === '/profile' ? 'page' : undefined}>
